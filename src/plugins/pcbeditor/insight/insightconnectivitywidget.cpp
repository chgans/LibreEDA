    ...
    QFrame(parent)
{
    ...
    for (int i = 0; i < 1; ++i)
    {
        ...
        itemi->setIcon(0, QIcon(":/icons/PCB-icon.png"));
        for (int j = 0; j < 2; ++j)
        {
            QTreeWidgetItem *itemj = new QTreeWidgetItem(itemi,
                                                         QStringList(QString("Scematics %1.%2").arg(i).arg(j)));
            itemj->setIcon(0, QIcon(":/icons/chip-package.png"));
        }
    }
